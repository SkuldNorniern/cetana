...
pub fn calculate_binary_cross_entropy_loss(predictions: &Tensor, targets: &Tensor) -> MlResult<f32> {
...
mod tests {
    ...
    #[test]
    fn test_mse_invalid_shapes() {
        let predictions = Tensor::new(vec![vec![1.0, 0.0]]).unwrap();
    ...
    #[test]
    fn test_binary_cross_entropy_invalid_shapes() {
        let predictions = Tensor::new(vec![vec![0.5]]).unwrap();
    ...
    fn test_binary_cross_entropy_batch() -> MlResult<()> {
        let predictions = Tensor::new(vec![
            vec![0.9, 0.1],
            vec![0.1, 0.9],
        ])?;
