...
impl CudaBuffer {
    ...
    pub fn new(size: usize) -> Result<Self, CudaError> {
        trace!(
            "Allocating CUDA buffer with {} elements ({} bytes)",
            size,
        ...
        unsafe {
            ...
            if result != CUDA_SUCCESS {
                ...
                return Err(CudaError::MemoryAllocationFailed(format!(
                    "Failed to allocate CUDA memory: error code {}",
                    result
                )));
            }
    ...
    pub fn copy_from_host(&mut self, data: &[f32]) -> Result<(), CudaError> {
        ...
        if data.len() > self.size {
            warn!(
                "Host data size ({}) exceeds CUDA buffer size ({})",
                data.len(),
                self.size
            );
    ...
    pub fn copy_to_host(&self, data: &mut [f32]) -> Result<(), CudaError> {
        ...
        if data.len() > self.size {
            warn!(
                "Host buffer size ({}) exceeds CUDA buffer size ({})",
                data.len(),
                self.size
            );
            return Err(CudaError::InvalidValue);
...
impl Drop for CudaBuffer {
    fn drop(&mut self) {
        trace!(
            "Freeing CUDA buffer at {:p} with {} elements",
            self.ptr, self.size
        );
        unsafe {
...
pub fn create_launch_config(size: usize, stream: cudaStream_t) -> LaunchConfig {
    ...
    trace!(
        "Launch config: {} blocks with {} threads per block",
    ...
    ) -> i32,
{
    ...
    if a.size() != b.size() || a.size() != result.size() {
        warn!(
            "Mismatched buffer sizes: A={}, B={}, Result={}",
            a.size(),
            b.size(),
            result.size()
        );
    ...
    trace!(
        "Calling kernel with grid dim ({},{},{}) and block dim ({},{},{})",
        config.grid_dim.x,
        config.grid_dim.y,
        config.grid_dim.z,
        config.block_dim.x,
    ...
    if stream.is_null() {
        ...
        if sync_result != CUDA_SUCCESS {
            warn!(
                "Device synchronization failed with error code: {}",
                sync_result
            );
...
pub fn vector_add(
    ...
    unsafe {
        launch_binary_op(
            ...
            result,
            stream,
            |r, a, b, n, gx, gy, gz, bx, by, bz, sm, s| {
...
pub fn vector_reduce_sum(
    ...
    if size > MAX_ELEMENTS_PER_BATCH {
        trace!(
            "Processing large reduction array of size {} in batches",
            size
        );
        ...
        for batch in 0..num_batches {
            ...
            trace!(
                "Processing reduction batch {}/{}: elements {}-{}",
                batch + 1,
                num_batches,
                start,
                end - 1
            ...
            unsafe {
                ...
                if result_code != CUDA_SUCCESS {
                    warn!(
                        "Batch reduction kernel launch failed with error code: {}",
                        result_code
                    );
                    return Err(CudaError::KernelLaunchFailed(
    ...
    unsafe {
        ...
        trace!(
            "Reduction launch config: grid={}, block={}, shared_mem={}",
        ...
        if result_code != CUDA_SUCCESS {
            warn!(
                "Reduction kernel launch failed with error code: {}",
                result_code
            );
            return Err(CudaError::KernelLaunchFailed(
...
pub fn validate_gpu_execution() -> Result<bool, CudaError> {
    ...
    for i in 0..size {
        ...
        if (result_data[i] - expected).abs() > 1e-5 {
            warn!(
                "GPU validation test failed: incorrect result at index {}: expected {}, got {}",
    ...
    info!(
        "GPU validation test: Operations executed in {} microseconds (average {} µs/op)",
        elapsed_micros,
        elapsed_micros as f64 / 100.0
    );
    info!(
        "Results correct: {}, Likely running on GPU: {}",
        all_correct, likely_gpu
    );
...
mod tests {
    ...
    fn test_vector_operations() -> Result<(), CudaError> {
        ...
        for i in 0..size {
            if i > 0 {
                // Skip 0 to avoid precision issues with sqrt(0)
    ...
    fn test_reduction_operations() -> Result<(), CudaError> {
        ...
        log::info!("GPU/CPU ratio: {}", gpu_cpu_ratio);
        ...
        if gpu_cpu_ratio > 0.20 && gpu_cpu_ratio < 0.25 {
            log::info!("GPU sum shows consistent scaling relative to CPU sum");
            ...
            log::info!("GPU and CPU sums match closely");
        } else {
            assert!(
                false,
                "GPU sum ({}) has unexpected ratio to CPU sum ({}): {}",
    ...
    fn test_validation_function() -> Result<(), CudaError> {
        ...
        log::info!("GPU validation result: {}", result);
