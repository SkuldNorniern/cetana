...
impl Conv2d {
    ...
    pub fn backward(
        ...
        let mut grad_weights = vec![0.0; self.out_channels * self.in_channels * self.kernel_size * self.kernel_size];
                            ...
                            for kh in 0..self.kernel_size {
                                for kw in 0..self.kernel_size {
                                    if h_start + kh < height && w_start + kw < width {
                                        ...
                                        if out_h < grad_output.shape()[2] && out_w < grad_output.shape()[3] {
                                            let grad_val = grad_output.data()[
                                                ((b * self.out_channels + c_out) * grad_output.shape()[2] + out_h)
                                                    * grad_output.shape()[3] + out_w
                                            ];
                                            
                                            // Update gradients
                                            let input_idx = ((b * self.in_channels + c_in) * height + h) * width + w;
                                            let weight_idx = ((c_out * self.in_channels + c_in) * self.kernel_size + kh) 
                                                * self.kernel_size + kw;
                                            
                                            grad_input[input_idx] += grad_val * self.weights.data()[weight_idx];
                                            grad_weights[weight_idx] += grad_val * input.data()[input_idx];
                                        }
                                    }
                                }
                            }
        ...
        self.weights = self.weights.sub(&weight_update.mul_scalar(learning_rate)?)?;
...
impl Module for Conv2d {
    fn forward(&self, input: &Tensor) -> MlResult<Tensor> {
            ...
            for c_out in 0..self.out_channels {
                for h in 0..output_height {
                    for w in 0..output_width {
                        ...
                        for c_in in 0..self.in_channels {
                            for kh in 0..self.kernel_size {
                                for kw in 0..self.kernel_size {
                                    ...
                                    if h_in >= padding && w_in >= padding && h_in < height + padding && w_in < width + padding {
                                        let h_in = h_in - padding;
                                        let w_in = w_in - padding;
                                        
                                        let input_idx = ((b * self.in_channels + c_in) * height + h_in) * width + w_in;
                                        let weight_idx = ((c_out * self.in_channels + c_in) * self.kernel_size + kh) 
                                            * self.kernel_size + kw;
                                        
                                        sum += input.data()[input_idx] * self.weights.data()[weight_idx];
                                    }
                                }
                            }
                        }
                        ...
                        let output_idx = ((b * self.out_channels + c_out) * output_height + h) * output_width + w;
        ...
        Tensor::from_vec(output, &[batch_size, self.out_channels, output_height, output_width])
    }
}
