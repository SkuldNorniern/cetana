use cetana::{
    ...
    MlError,
    tensor::Tensor,
    nn::{Linear, Module, activation::{Sigmoid, ReLU}},
    serialize::{Model, SerializeComponents, DeserializeComponents, Serialize, Deserialize},
...
fn main() -> MlResult<()> {
    ...
    // Load data from CSV
    let (heights, weights, labels) = load_data().map_err(|e| MlError::StringError(e.to_string()))?;
    ...
    for i in 0..x_test.data().len() / 2 {
        ...
        let height = x_test.data()[height_idx] * (heights.iter().fold(Float::MIN, |a, &b| a.max(b)) - 
                                                 heights.iter().fold(Float::MAX, |a, &b| a.min(b))) +
                     heights.iter().fold(Float::MAX, |a, &b| a.min(b));
        let weight = x_test.data()[weight_idx] * (weights.iter().fold(Float::MIN, |a, &b| a.max(b)) - 
                                                 weights.iter().fold(Float::MAX, |a, &b| a.min(b))) +
                     weights.iter().fold(Float::MAX, |a, &b| a.min(b));
        
        let raw_pred = predictions.data()[i];
        let predicted = if raw_pred > 0.5 { "Female" } else { "Male" };
        let actual = if y_test.data()[i] > 0.5 { "Female" } else { "Male" };
        
        println!("║  {:6.1}  │  {:6.1}  │      {:.4}      │   {:8}  │  {:8}  ║",
            height,
            weight,
            raw_pred,
            predicted,
